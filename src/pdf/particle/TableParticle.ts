...
export class TableParticle {
  ...
  private _drawBorder(ctx: Context2d, startX: number, startY: number, width: number, height: number) {
    ctx.beginPath()
  ...
  public render(ctx: Context2d, element: IElement, startX: number, startY: number) {
    const { colgroup, trList } = element
    if (!colgroup || !trList) return
